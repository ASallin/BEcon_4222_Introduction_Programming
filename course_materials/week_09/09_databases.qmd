...
> **Note:** Need 3 is enforced through **ACID** properties (Atomicity, Consistency, Isolation, Durability).
...
- **Understanding** the business, tailored for **analytics** and research.
- ***Data warehousing***
...
::: {.columns}
::: {.column width="40%"}
![](../../assets/img/duckdb_logo.png){width="90%"}
:::
...
- OLAP database built for analytics and research
...
- DuckDB is both usable with Python and R. The [project.db]{.path} file can be accessed from both.
...
::: {.aside}
Aubury & Letcher, *Getting Started with DuckDB* (Packt, 2024), Ch. 1–3.
:::
...
![](../../assets/img/lecture_09_db/oltp_olap_kleppmann.png){fig-align="center" width="100%"}
...
::: {.aside}
Source: [Kleppmann, 2017, p. 88](https://0-lucas.github.io/digital-garden/99.-Books/Martin-Kleppmann---Designing-Data-Intensive-Applications_-O%E2%80%99Reilly-Media-(2017).pdf)
:::
...
> **Another pipeline**: ELT: Extract, Load, Transform. Loads raw data directly into a warehouse before transforming it, enabling faster loading and greater flexibility than traditional ETL. Key usage examples include real-time analytics, big data processing, and centralizing data from diverse sources.
...
2. Populate/Load the model with the **transformed** data
...
> **Note:** Refer to the documentation of each SQL dialect. [Link for DuckDB](https://duckdb.org/docs/current/sql/data_types/overview)
...
#### Today, we cover the basics of `SELECT`, `FROM`, `WHERE`, `ORDER BY`, and `LIMIT`. In the next lecture, we will cover more advanced clauses like `GROUP BY`, `HAVING`, and `JOIN`.
...
- The condition can reference any column from the `FROM` tables, but not columns defined in the `SELECT` clause (because `SELECT` is evaluated after `WHERE`).
...
> The database executes `FROM` → `WHERE` → `SELECT` → `ORDER BY` → `LIMIT`. `SELECT` is written first but evaluated last. This is why you can use a column alias in `ORDER BY` but not in `WHERE`.
...
- SQL has other statements for modifying data and schema, such as `UPDATE`, `ALTER TABLE`, and `DELETE`.
- `ALTER TABLE` modifies the structure of the table (e.g. add a column, change a type).
- `UPDATE` modifies existing rows (e.g. set `gdp_usd = gdp_usd * 1.1` to adjust for inflation).
- `DELETE` removes rows from a table (e.g. `DELETE FROM countries WHERE name = 'Switzerland'`).
...
# Create a connection to a .db file (creates the file if it doesn't exist)
...
- DuckDB supports a large subset of the SQL standard, with some extensions.
...
- Run queries using `conn.execute("SQL QUERY").df()` in Python, and close the connection when done.
- We usually convert the result to a DataFrame for further analysis in Python, appending `.df()` at the end of the query.
...
- Use `EXTRACT(YEAR FROM date_column)` to get the year from a date variable. Works with `MONTH`, `DAY`, etc. Example: `WHERE EXTRACT(YEAR FROM date) = 2020`
...
- Kleppmann's *Designing Data-Intensive Applications*: https://0-lucas.github.io/digital-garden/99.-Books/Martin-Kleppmann---Designing-Data-Intensive-Applications_-O%E2%80%99Reilly-Media-(2017).pdf
...
## 🔎 Self-study: DuckDB is not a classic RDBMS
...
> **Note:** As of DuckDB 1.0+, `CASCADE` and `SET NULL` are supported. In older versions, only `RESTRICT` was available. They are standard in PostgreSQL, MySQL, and SQLite.
